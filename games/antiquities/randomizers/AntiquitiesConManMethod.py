...
#Check for Shelters using ConMan's "Method 1" with triple the card draws
random.shuffle(resultList)
#make a pool of all the non-included cards
method1list = list(set(pullList) - set(resultList))
random.shuffle(method1list)
#grab 3 random cards from the Kingdom and 3 cards from the non-included cards
#check to see if any of them are Dark Ages
print('Shelters: ' + str((set(resultList[:3]) | set(method1list[:3]))))
includeShelters = (set(resultList[:3]) | set(method1list[:3])) & set(DarkAges)
...
random.shuffle(resultList)
random.shuffle(method1list)
print('Colonies: ' + str((set(resultList[:3]) | set(method1list[:3]))))
includeColPlat = (set(resultList[:3]) | set(method1list[:3])) & set(Prosperity)

#Check for Boulder traps
random.shuffle(resultList)
random.shuffle(method1list)
print('Boulder Traps: ' + str((set(resultList[:3]) | set(method1list[:3]))))
includeBTraps = (set(resultList[:3]) | set(method1list[:3])) & set(Antiquities)
